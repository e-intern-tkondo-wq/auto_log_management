# 変更履歴
> このファイルで行うこと: 実装済みの機能追加・修正の履歴を残します。

## 2025-12-XX: LLM自動解析機能の実装

### 追加機能

1. **LLM自動解析機能**
   - `src/llm_analyzer.py` を追加
   - 未知ログをLLM（GPT）で自動解析
   - 異常と判断した場合: アラートを自動作成
   - 正常と判断した場合: パターンを自動追加
   - `.env` ファイルからのAPIキー読み込みに対応

2. **ai_analyses テーブル**
   - LLM解析結果を保存するテーブルを追加
   - `prompt`, `response`, `model_name` を保存

3. **デフォルト値の変更**
   - `regex_patterns.label` のデフォルト: `'unknown'` → `'normal'`
   - `log_entries.classification` のデフォルト: `'unknown'` → `'normal'`
